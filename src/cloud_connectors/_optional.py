...
Usage:
    from cloud_connectors._optional import require_extra, is_available
    ...
    require_extra("boto3", "aws")  # -> "Install with: pip install cloud-connectors[aws]"
"""
...
PACKAGE_INSTALL_HINTS: dict[str, str] = {
    "crewai": (
        "Install CrewAI separately after reviewing its dependency tree; cloud-connectors does not publish a "
        "CrewAI extra while current CrewAI releases pull vulnerable chromadb versions."
    ),
    "sentence_transformers": (
        "Install sentence-transformers separately after reviewing its dependency tree; cloud-connectors does not "
...
def require_extra(package: str, extra: str | None = None) -> Any:
    ...
    try:
        ...
        raise ImportError(
            f"Package '{package}' is required but not installed.\n"
            f"Install with: pip install cloud-connectors[{extra_name}]"
        ) from e


def get_crewai_tool_decorator() -> Any:
    """Import the CrewAI tool decorator with cloud-connectors install guidance."""
    try:
...
def require_any(*packages: str, extra: str) -> Any:
    ...
    raise ImportError(
        ...
        f"Install with: pip install cloud-connectors[{extra}]"
    )
...
def get_connector_install_command(connector: str) -> ExtendedString | None:
    ...
    if extra is None:
        return None
    return ExtendedString(f"pip install cloud-connectors[{extra}]")
