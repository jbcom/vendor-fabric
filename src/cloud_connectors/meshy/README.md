...
Meshy support is part of `cloud-connectors` and lives under
`cloud_connectors.meshy`. It provides functional API helpers, a
...
```bash
pip install "cloud-connectors[meshy,vector]"
```
...
from cloud_connectors.meshy.models import ArtStyle, Text3DRequest
...
modules from `cloud_connectors.meshy`.

## Connector Fabric

```python
from cloud_connectors import ConnectorFabric
from cloud_connectors.meshy import create_meshy_logger
...
```python
from cloud_connectors.meshy.jobs import AssetGenerator, example_character_spec
...
```python
from cloud_connectors.meshy.webhooks import WebhookHandler
...
```python
from cloud_connectors.meshy.tools import get_langchain_tools, get_strands_tools, get_tools
